...
keytool -keystore "${DIR}/trust.jks" -importcert -file "${DIR}/root/ca.crt" -alias root -storepass password
keytool -keystore "${DIR}/trust.jks" -importcert -file "${DIR}/intermediate/intermediate.crt" -alias intermediate -storepass password
...
openssl pkcs12 -export -in "${DIR}/mth.crt" -inkey "${DIR}/mth.key" -out "${DIR}/mth.p12" -passout pass:password
keytool -importkeystore -deststorepass password -destkeystore "${DIR}/mth.jks" -srckeystore "${DIR}/mth.p12" -srcstorepass password -srcstoretype PKCS12

# Generate a CSR using the configuration and the key just generated. We will
# give this CSR to our CA to sign.
echo "Create Test system certificate"

cat > "${DIR}/test.cnf" << EOF
[req]
default_bits = 2048
encrypt_key  = no # Change to encrypt the private key using des3 or similar
default_md   = sha256
...
openssl pkcs12 -export -in "${DIR}/test.crt" -inkey "${DIR}/test.key" -out "${DIR}/test.p12" -passout pass:password
keytool -importkeystore -deststorepass password -destkeystore "${DIR}/test.jks" -srckeystore "${DIR}/test.p12" -srcstorepass password -srcstoretype PKCS12
